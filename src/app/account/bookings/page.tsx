...
export default function AccountBookingsPage() {
 ...
 {bookings.map((booking) => (
 <div key={booking.title} className=" border border-slate-200 bg-slate-50/60 p-4">
 <div className="flex items-start justify-between gap-3">
 <h4 className="font-bold text-slate-900 text-sm">{booking.title}</h4>
 <span className={`shrink-0 inline-block px-2.5 py-1 text-[10px] font-extrabold border ${booking.statusColor}`}>
 {booking.status}
 </span>
 </div>
 ...
 <p className="font-semibold text-slate-600">{booking.consultant}</p>
 </div>
